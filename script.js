...
document.getElementById('makeWish').addEventListener('click', e => { const cake = document.getElementById('cakeWrap'), magic = document.getElementById('wishMagic'); if(cake.classList.contains('blown')) return; cake.classList.add('blown'); magic.classList.add('active'); document.body.classList.add('celebrate'); document.getElementById('cakeResult').classList.add('show'); e.currentTarget.textContent='Điều ước đang lên đường'; for(let i=0;i<88;i++){ const spark=document.createElement('i'); spark.className='wish-spark'; spark.style.setProperty('--x',`${(Math.random()-.5)*105}vw`); spark.style.setProperty('--y',`${-18-Math.random()*70}vh`); spark.style.setProperty('--d',`${1.7+Math.random()*1.7}s`); spark.style.setProperty('--s',`${3+Math.random()*7}px`); spark.style.background=['#f7d99b','#fff5da','#ddabb0','#d9b77e'][i%4]; magic.appendChild(spark); setTimeout(()=>spark.remove(),4000); } setTimeout(()=>document.body.classList.remove('celebrate'),4400); });
